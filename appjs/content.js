...
chrome.extension.onRequest.addListener(function(request, sender, sendResponse) {
	if(request.action=="caiji"){
		...
		var weixinData={
			...
			weixin_type   : "",
			verify_type   : "",
			caiji_time    : myDate.getFullYear()+"-"+(myDate.getMonth()+1)+"-"+myDate.getDate()+" "+myDate.getHours()+":"+myDate.getMinutes()+":"+myDate.getSeconds(),
			weixin_date   : wxDate,
			user_data     : null,
			article_data  : null,
		...
		/*其他数据*/
		var liList;
		var index_tap_added=document.getElementsByClassName("index_tap added").item(0);
		liList=index_tap_added.getElementsByTagName("li");
		weixinData.added_message=liList.item(0).getElementsByTagName("em").item(0).innerHTML;
		weixinData.added_fans=liList.item(1).getElementsByTagName("em").item(0).innerHTML;
		var index_tap_total=document.getElementsByClassName("index_tap total").item(0);
		liList=index_tap_total.getElementsByTagName("li");
		weixinData.total_fans=liList.item(0).getElementsByTagName("em").item(0).innerHTML;
		/*获取token*/
